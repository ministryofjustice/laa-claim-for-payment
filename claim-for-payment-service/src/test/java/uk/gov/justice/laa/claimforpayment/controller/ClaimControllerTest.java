...
import uk.gov.justice.laa.claimforpayment.config.TestOauthConfig;
...
@Import({
  ...
  TestOauthConfig.class
}) // Import security and OAuth2 config for tests
class ClaimControllerTest {
  ...
  void getClaims_returnsOkStatusAndAllClaims() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/v1/submissions/123e4567-e89b-12d3-a456-426614174000/claims")
                .with(jwt().authorities(() -> "SCOPE_api.read")))
  ...
  //     when(mockClaimService.createClaim(any(UUID.class),
  // any(ClaimRequestBody.class))).thenReturn(3L);
  ...
  //     verify(mockClaimService, never()).createClaim(any(UUID.class),
  // any(ClaimRequestBody.class));
  //   }
